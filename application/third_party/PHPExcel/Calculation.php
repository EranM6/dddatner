...
class PHPExcel_Calculation
{
    ...
    /**
     * Locale-specific translations for ExcelFile constants (True, False and Null)
     *
     * @var string[]
     *
     */
    ...
    /**
     * ExcelFile constant string translations to their PHP equivalents
    ...
    {
        //    Convert any ExcelFile function names to the required language
    ...
    private function convertMatrixReferences($formula)
    {
        ...
        //    Convert any ExcelFile matrix references to the MKMATRIX() function
        if (strpos($formula, '{') !== false) {
            //    If there is the possibility of braces within a quoted string, then we don't treat those as matrix indicators
            if (strpos($formula, '"') !== false) {
                //    So instead we skip replacing in any quoted strings by only replacing in every other array element after we've exploded
...
//                        echo 'Element '.$excelConstant.' is an ExcelFile Constant<br />';
                        $val = self::$excelConstants[$excelConstant];
                    } elseif (($localeConstant = array_search(trim(strtoupper($val)), self::$localeBoolean)) !== false) {
//                        echo 'Element '.$localeConstant.' is an ExcelFile Constant<br />';
...
//                    } else {
                ...
                // if the token is a number, boolean, string or an ExcelFile error, push it onto the stack
...
//                    echo 'Token is a number, boolean, string, null or an ExcelFile error<br />';
    ...
    private function validateBinaryOperand($cellID, &$operand, &$stack)
    {
        ...
        if (is_string($operand)) {
            ...
            if (!is_numeric($operand)) {
                //    If not a numeric, test to see if the value is an ExcelFile error, and so can't be used in normal binary operations
                if ($operand > '' && $operand{0} == '#') {
...
//            echo '<br />';
    ...
    /**
     * Get a list of all ExcelFile function names
     *
     * @return    array
     */
    ...
    /**
     * Get a list of implemented ExcelFile function names
     *
     * @return    array
     */
